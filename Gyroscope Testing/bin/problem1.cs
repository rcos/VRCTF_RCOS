...
// Command-line arguments: build --output bin\problem1 c:\Users\lianga4\Downloads\PSOFT CONTENT\problem1.dfy
// problem1.dfy

method psp(x: int, y: int)
    returns (x': int, y': int)
  ensures x' + y' == x + y && x' * y' < 0
  decreases x, y
{
  if (x > 0 && y < 0) || (x < 0 && y > 0) {
    x' := x;
...
namespace _module {
  ...
  public partial class __default {
    public static void psp(BigInteger x, BigInteger y, out BigInteger x_k, out BigInteger y_k)
    {
      x_k = BigInteger.Zero;
      y_k = BigInteger.Zero;
      if ((((x).Sign == 1) && ((y).Sign == -1)) || (((x).Sign == -1) && ((y).Sign == 1))) {
        x_k = x;
        y_k = y;
      } else if ((((x).Sign == 0) || ((x).Sign == 1)) && (((y).Sign == 0) || ((y).Sign == 1))) {
        if (((x).Sign == 0) && ((y).Sign == 0)) {
          x_k = BigInteger.One;
          y_k = new BigInteger(-1);
        } else {
          x_k = ((x) + (y)) + (BigInteger.One);
          y_k = new BigInteger(-1);
        }
      } else if ((((x).Sign == 0) || ((x).Sign == -1)) && (((y).Sign == 0) || ((y).Sign == -1))) {
        if (((x).Sign == 0) && ((y).Sign == 0)) {
