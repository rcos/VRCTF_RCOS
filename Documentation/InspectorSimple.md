...
## Overview
The script in `Assets/Scripts/InspectControllerSimple.cs` contains everything needed to inspect an object.

Attach `InspectController.cs` to any GameObject in the `Interactible` layer. In the Inspector window's Script Component. The inspect location is set to 1 unit higher than the starting position. Also
